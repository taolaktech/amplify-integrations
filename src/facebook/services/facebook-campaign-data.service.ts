...
export class FacebookCampaignDataService {
  ...
  processCampaignDataForFacebook(campaignData: CampaignDataFromLambda): {
    ...
    try {
      ...
      // Validate Facebook is in platforms
      if (!campaignData.platforms.includes('FACEBOOK')) {
        throw new Error(
          `Campaign ${campaignData.campaignId} does not target Facebook platform`,
        );
      }
  ...
  async createFacebookCampaignFromLambda(
    ...
    try {
      ...
      // We need to fetch the full ad account details here to get the pixelId
      const primaryAdAccount = await this.facebookAdAccountModel
        .findOne({
          userId: campaignData.userId,
          accountId: userAdAccountId,
          isPrimary: true,
          integrationStatus: 'READY_FOR_CAMPAIGNS',
        })
        .lean();

      if (!primaryAdAccount || !primaryAdAccount.metaPixelId) {
        throw new BadRequestException(
          `User ${campaignData.userId} has no configured Meta Pixel ID for ad account ${userAdAccountId}.`,
        );
      }
      const userMetaPixelId = primaryAdAccount.metaPixelId;
